...
// Cro Token Staking Contract

contract CroStaking is Ownable {
    ...
    IBEP20 public immutable croToken;

    uint256 private lastRewardBlock;

    uint256 private accCROPerShare;
    ...
    function init() private {
        UnStakeFee memory _UnStakeFee1 = UnStakeFee({
            minDays : 7,
            feePercent : 15
        });

        UnStakeFee memory _UnStakeFee2 = UnStakeFee({
            minDays : 15,
            feePercent : 10
        });

        UnStakeFee memory _UnStakeFee3 = UnStakeFee({
            minDays : 30,
            feePercent : 5
        });
        unStakeFees.push(_UnStakeFee1);
        unStakeFees.push(_UnStakeFee2);
        unStakeFees.push(_UnStakeFee3);
    }

    function setBountyWallet(address _bountyWallet) external onlyOwner {
        bountyAddress = _bountyWallet;
        emit SetBountyWallet(bountyAddress);        
    }
    ...
    function setUnStakeFee(uint256 _index, uint256 _minDays, uint256 _feePercent) external onlyOwner {
        ...
        require(_feePercent > 0, "ERROR: Invalid feePercent");
    ...
    function removeUnStakeFee(uint256 _index) external onlyOwner {
        ...
        for (uint256 i = _index; i < unStakeFees.length-2; i++) {
    ...
    function _getPending(address _user) private view returns (uint256) {
        ...
        uint256 acc = accCROPerShare;
        if(block.number > lastRewardBlock && totalStakedAmount != 0) {
            uint256 multiplier = getMultiplier(lastRewardBlock, block.number);
            uint256 reward = multiplier.mul(rewardPerBlock);
            acc = acc.add(reward.mul(1e12)).div(totalStakedAmount);
        }
        return user.amount.mul(acc).div(1e12).div(1e12).sub(user.rewardDebt).add(user.pendingAmount);
    }

    function getRewardBalance() external view returns (uint256) {
        if(block.number > lastRewardBlock && totalStakedAmount != 0) {
            uint256 multiplier = getMultiplier(lastRewardBlock, block.number);
            uint256 reward = rewardPerBlock.mul(multiplier);
    ...
    function depositReward(uint256 _amount) external onlyOwner {
        croToken.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function updateStatus() private {
        ...
        accCROPerShare = accCROPerShare.add(reward.mul(1e12).div(totalStakedAmount));
        lastRewardBlock = block.number;
    }
    ...
    function stake(uint256 _amount) public {
        ...
        updateStatus();
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(accCROPerShare).div(1e12).sub(user.rewardDebt);
            user.pendingAmount = user.pendingAmount.add(pending);
        }
        croToken.safeTransferFrom(msg.sender, address(this), _amount);
        totalStakedAmount = totalStakedAmount.add(_amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(accCROPerShare).div(1e12);
    ...
    function unStake(uint256 _amount) public {
        ...
        updateStatus();
        uint256 pending = user.amount.mul(accCROPerShare).div(1e12).sub(user.rewardDebt);
        if (croToken.balanceOf(address(this)) < pending) {
            pending = croToken.balanceOf(address(this));
        }
        user.pendingAmount = user.pendingAmount.add(pending);
        ...
        croToken.safeTransfer(msg.sender, amount);
        croToken.safeTransfer(bountyAddress, unStakeFee);
    ...
    function harvest(bool reStake) public {
        ...
        if(reStake) {
            ...
            if(croToken.balanceOf(address(this)) < amount) {
                amount = croToken.balanceOf(address(this));
            }

            if(croToken.balanceOf(address(this)) < _harvestFee) {
                _harvestFee = croToken.balanceOf(address(this));
            }

            croToken.safeTransfer(msg.sender, amount);
            croToken.safeTransfer(bountyAddress, _harvestFee);
            emit Harvest(msg.sender, amount, _harvestFee);

            updateStatus();
            user.pendingAmount = 0;
            user.rewardDebt = user.amount.muli(accCROPerShare).div(1e12);
        }
    }
}